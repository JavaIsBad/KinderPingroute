...
void pingerICMP(){
	...
	icmpPacket->icmp_cksum=checksum(icmpPacket, 8+sizeData);
	i=sendto(sockfd, packet, 8+sizeData, 0, (struct sockaddr*) &destination, sizeof(struct sockaddr));
	
	if(i<0 || i< 8+sizeData){
		if(i<0)
			perror("sendto :");
		fprintf(stderr, "ping : sendto %s %d chars, achieve %d\n", hostname, 8+sizeData, i);
		fflush(stderr);
	}
	nbrSend++;
}
...
void sigIntAction(int signum){
	...
	if(nbrReceive>0)
		printf("round-trip time (ms) min/max/avg =>  %d/%d/%d\n", timeMin, timeMax, timeOverall/nbrReceive);
	fflush(stdout);
	exit(EXIT_SUCCESS);
}
...
int main(int argc, char** argv){
	...
	for(;;){
		int doctorWhoLength=sizeof(from);
		int nbrecv;
